...
if st.button("🚀 开始分析"):
    if not all([movie_title, yt_api_key, deepseek_api_key]):
        ...
        if err:
            ...
            df_result['sentiment_show'] = df_result['sentiment'].map(lambda x: valmap.get(str(x).capitalize(), x))
            df_result['sentiment_show'].value_counts().plot.pie(autopct='%.1f%%', ax=ax1)
            ax1.set_title('评论情感分布')
            ax1.set_ylabel('')
            st.pyplot(fig1)

            st.subheader("2. 每日评论情感趋势")
            df_result['date'] = df_result['published_at_hk'].dt.date
            daily = df_result.groupby(['date', 'sentiment_show']).size().unstack().fillna(0)
            fig2, ax2 = plt.subplots(figsize=(10,4))
            daily.plot(kind='bar', stacked=True, ax=ax2)
            ax2.set_title('每日评论情感趋势')
            ax2.set_xlabel('日期')
            ax2.set_ylabel('评论数')
            st.pyplot(fig2)

            st.subheader("3. 下载分析明细")
